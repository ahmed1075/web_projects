...
if (isset($_SESSION['email'])) {
    ...
    if (isset($_POST['selecte'])) {
        ...
        $sql = mysqli_query($conn, "UPDATE sourcing SET status='$show' where sourcing_id='$id'");
        if ($sql) {
            if ($show == 'Success') {
                ?>
                <script>
                    window.alert('Source set to Successed');
                ...
                ?>
                <script>
                    window.alert('Source set to Pending');
                    window.location.href = 'sourcing.php';
                </script>
                ...
                ?>
                <script>
                    window.alert('Source set to Failed');
                    window.location.href = 'sourcing.php';
                </script>
                                ...
                                <div class="row table-responsive">
                                    <table class="table table-bordered" id="myTable" style="text-align:center">
                                        ...
                                        <tbody style="font-family:raleway">
                                            <?php
                                            $sql = mysqli_query($conn, "SELECT * FROM sourcing");
                                            while ($row = mysqli_fetch_assoc($sql)) {
                                                ?>
                                                <tr>
                                                    <td><b><?php echo $row['date']; ?></b></td>
                                                    <td><b><?php echo $row['sourcing_id'] ?></b></td>
                                                    <td><b><?php echo $row['product_url'] ?></b></td>
                                                    <td><b><?php echo $row['price'] ?></b></td>

                                                    <td>
                                                        <div class="col-lg-6 offset-lg-3">
                                                            <form method="post" action="sourcing.php">
                                                                <div class="input-group">
                                                                    <input type="hidden" name="id" class="form-control" value="<?php echo $row['sourcing_id']; ?>">
                                                                    <select class="form-control" name="select" style="background-color:#e8f1ff;width:200px">
                                                                        <option <?php if ($row['status'] == 'Success') echo 'selected'; ?> value="Success">Success</option>
                                                                        <option <?php if ($row['status'] == 'Pending') echo 'selected'; ?> value="Pending">Pending</option>
                                                                        <option <?php if ($row['status'] == 'Failed') echo 'selected'; ?> value="Failed">Failed</option>
