...
// $con = mysqli_connect("localhost", "root", "", "notif");
        if ($_POST["view"] != '') {
            $update_query = "UPDATE fullpaper SET comment_status = 1 WHERE comment_status=0";
            mysqli_query($con, $update_query);
        }
        $query = "SELECT * FROM fullpaper where sub_theme='$dept' ORDER BY id DESC LIMIT 5";
  ...
  <li>
  <a href="#">
  <strong>' . $row["paper"] . '</strong><br />
  <small><em>' . $row["sub_theme"] . '</em></small>
  </a>
  </li>
  ';
            }
